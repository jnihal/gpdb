...
func (s *Server) AddMirrors(req *idl.AddMirrorsRequest, stream idl.Hub_AddMirrorsServer) error {
	...
	err = ValidateDataChecksums(s.Conns, gparray)
	if err != nil {
		return utils.LogAndReturnError(err)
	}
	...
	err = s.UpdatePgHbaConf(gparray, req.Mirrors, true)
	if err != nil {
		return utils.LogAndReturnError(err)
	}
...
func (s *Server) UpdatePgHbaConf(gparray *greenplum.GpArray, mirrorSegs []*idl.Segment, hbaHostname bool) error {
	...
	request := func(conn *Connection) error {
		...
		for _, pair := range pairs {
			...
			go func(pair *greenplum.SegmentPair) {
				...
				if hbaHostname {
					addrs = []string{pair.Mirror.Hostname}
				} else {
					addrs, err = s.GetInterfaceAddrs(pair.Mirror.Hostname)
					if err != nil {
						errs <- err
						return
					}
...
func ValidateDataChecksums(conns []*Connection, gparray *greenplum.GpArray) error {
