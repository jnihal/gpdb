...
def impl(context, process_name, signal_name):
    ...
    command = "ps ux | grep bin/{0} | awk '{{print $2}}' | xargs kill -{1}".format(process_name, sig)
    run_async_command(context, command)
...
def impl(context, command, input, delay):
    ...
    context.execute_steps(u'''Then verify that pg_basebackup is running for content 0''')
    p.send_signal(signal.SIGINT)

    if delay == "with":
        context.execute_steps(u'''When the user waits until mirror on content 0 is up''')

    p.stdin.write(input.encode("utf-8"))
    p.stdin.flush()

    if input == "n":
        context.execute_steps(u'''Then the user reset the walsender on the primary on content 0''')
...
def impl(context, command):
    ...
    context.execute_steps(u'''Then the user just waits until recovery_progress.file is created in gpAdminLogs''')
